...
public class SearchResultItem {
	...
	public synchronized Object[] getField(String fieldName) {
		...
		if (f.storage==Field.Store.NO) throw new IllegalFieldConfigException("Field '"+fieldName+"' is not stored!");
	...
	public synchronized java.util.Map<String,Object[]> getFields() {
		...
		for (FieldConfig f : config.fields.values()) if (f.storage!=Field.Store.NO) {
	...
	private Object[] getField(FieldConfig f) {
		...
		if (data!=null) {
			...
			for (String val : data) try {
				switch(f.datatype) {
					case NUMBER:
						vals.add(Double.valueOf(config.trieImpl.trieCodedToDouble(val))); break;
					case DATETIME:
						vals.add(config.trieImpl.trieCodedToDate(val)); break;
					default:
						vals.add(val); break;
				}
