...
public final class Package {
  ...
  private static void throwVersionException(String name, String desired) {
    throw new RuntimeException(
        getProductName()
            ...
            + "If you are sure, that all is correct, you may disable this version checking by setting the Java system property '"
            + SKIP_VERSION_CHECK_PROPERTY + "' to 'true'.");
  }
  
  private static void checkPackage(java.lang.Package pkg, String name,
      String nameMatch, String desired) {
    String s;
    if (pkg == null
      ...
      log.warn("Cannot determine version of component '"
          ...
          + "'. "
          + getProductName()
          + " may work, but it would be better to check that a minimum version "
          + desired + " is installed in classpath!");
    } else if (!(isVersionCompatible(pkg.getSpecificationVersion(), desired) || isVersionCompatible(
        pkg.getImplementationVersion(), desired))) throwVersionException(name,
        desired);
    
  }
  
  private static void checkPackage(String clazz, String name, String nameMatch,
      String desired) {
    try {
   ...
   *           if the version of Lucene and other components is too old.
   */
  public static void checkMinimumRequirements() {
    if (!BooleanParser.parseBoolean(System.getProperty(
      ...
      checkPackage("org.apache.commons.digester.Digester",
          "Apache Commons Digester", "digester", "2.0");
    }
  }
