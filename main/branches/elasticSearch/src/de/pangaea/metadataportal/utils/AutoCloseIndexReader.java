...
/**
 * <code>AutoCloseIndexReader</code> is used by
 * {@link de.pangaea.metadataportal.config.IndexConfig#getSharedIndexReader} to
 * make IndexReaders cleanup by GC possible.
 * 
...
public final class AutoCloseIndexReader extends FilterDirectoryReader {
  ...
  private static org.apache.commons.logging.Log log = org.apache.commons.logging.LogFactory
      .getLog(AutoCloseIndexReader.class);
  
  public AutoCloseIndexReader(final IndexReader in, final String name) {
    this(in, name, false);
  }
  
  private AutoCloseIndexReader(final IndexReader in, final String name,
      final boolean reopened) {
    super(in);
    this.name = name;
    log.info((reopened ? "Softly reopened" : "Opened")
        + " shared reader of index '" + name + "'.");
  }
  
  @Override
  protected synchronized final void doClose() throws IOException {
    // the index may only be closed on finalization
    if (!finalizationStarted) throw new UnsupportedOperationException(
        "This IndexReader should not be closed.");
  ...
  protected IndexReader doOpenIfChanged(IndexWriter writer,
    ...
    final IndexReader n = IndexReader
        .openIfChanged(in, writer, applyAllDeletes);
    return (n == null) ? null : new AutoCloseIndexReader(n, name, true);
  }
  
  public final synchronized void hardClose() throws CorruptIndexException,
      IOException {
